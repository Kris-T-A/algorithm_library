...
// Combine a number of audio channels to one channel by taking the maximum value of each frequency band. The input audio signal is a matrix of size bufferSize x nChannels and
// the FFT size is 4 * bufferSize. The output audio signal is a vector of size bufferSize.
//
// author: Kristian Timm Andersen

struct AudioCombineMaxConfiguration
{
...
class AudioCombineMax : public AlgorithmImplementation<AudioCombineMaxConfiguration, AudioCombineMax>
{
  public:
  ...
  private:
    void processAlgorithm(Input input, Output output)
    {
        ...
        for (auto channel = 1; channel < C.nChannels; channel++)
        {
            ...
            {
                float power = std::norm(spectrogramOut(i, channel));
